...
export const FormGroup = ({children, half, inline}: FormGroupProps) => {
    return (
        <div
            className={`form-group flex ${inline ? 'flex-row' : 'flex-col'} ${half ? 'w-1/2' : 'w-full'} ${half ? 'gap-4' : 'gap-0'}`}>
            {children}
        </div>
    )
}
...
export const Input = ({label, subLabel, error, ...props}: InputProps) => {
    return (
        <>
            ...
            <input
                className="my-4 p-3 text-textLight font-light font-normal rounded-md border border-secondaryLighter focus:border-transparent focus:outline-none focus:ring-1 focus:ring-primary focus:border-transparent" {...props}/>
            {error &&
